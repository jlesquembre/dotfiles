...
#  * share a statusbar for the first awl item, you will need to manually
#    /statusbar window add -after lag -priority 10 awl_shared
#      left   : space in cells occupied on the left of statusbar
...
sub reset_awl {
	...
	$window_sort_func = sub {
	    ...
	    for (@sort_order) {
		my $x = reduce { $a->{$b} } $a, @{$_}[1..$#$_];
		my $y = reduce { $a->{$b} } $b, @{$_}[1..$#$_];
		return ((($x <=> $y || $x cmp $y) * $_->[0]) || next);
	    }
